...
Version: 0.1
Status: Agreed MVP and Extension Outline
...
Its job is to maintain the evolving domain knowledge structure for the user's intent and to produce the next non-executable research proposal that improves that knowledge.
...
- internet knowledge is planner input, not planner behavior
...
6. Produce the next-step proposal.
...
Internet-searched knowledge must be produced by a separate research agent.

The planner consumes that research as input alongside local knowledge.
...
4. It uses research input without confusing external knowledge with local proof.
5. It outputs only non-executable planning artifacts.
6. Its outputs are structured enough for downstream agents to consume deterministically.
